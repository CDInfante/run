...
/**
 * Custom map marker and control styling for Leaflet.
 * Includes support for glassmorphism and dark mode inversion.
 *
...
const mapStyles = `
  ...
  /* Scaled the popup smaller as requested */
  .leaflet-popup {
...
/**
 * Overrides for IPMA regional markers to ensure they are accurately
 * placed on the map.
 */
...
/**
 * Component to handle user geolocation, display a marker at the
 * user's current position, and initial centering.
 */
...
/**
 * Interactive Map Component using Leaflet and OpenStreetMap data.
 * Displays amenities (water, toilets) and official weather warnings.
 */
interface MapProps {
  /** Visibility toggle for water fountain markers */
  showWater: boolean;
  /** Visibility toggle for public toilet markers */
  showToilets: boolean;
  /** Visibility toggle for weather warning markers */
  showAlerts: boolean;
  /** Visibility toggle for trail markers */
  showTrails: boolean;
}
...
const Map: React.FC<MapProps> = ({
  ...
  const [portStatus, setPortStatus] = useState<ShipStatus | null>(null);
  const [userLocation, setUserLocation] = useState<L.LatLng | null>(null);
  const [mapInstance, setMapInstance] = useState<L.Map | null>(null);

  useEffect(() => {
    const loadData = async () => {
      const [amenityData, warningData, trailData, shipData] = await Promise.all(
        [
          fetchAmenities(),
          fetchWeatherWarnings(),
          fetch("/trails-madeira.json").then((res) =>
            res.json(),
          ) as Promise<TrailsData>,
          fetchShipStatus(),
        ],
      );
  ...
  return (
    <div className="h-full w-full relative z-10 group/map">
      ...
      >
        ...
        {/* Regional boundary Polygon logic removed as requested */}
